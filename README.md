...
`Minusone` is a deobfuscation engine focused on scripting language. `Minusone` is based on `tree-sitter` for parsing, and will apply a set of rules to infer node value.

`Minusone` is supporting the following languages:
* Powershell

By taking the example from [`Invoke-Obfuscation`](https://github.com/gh0x0st/Invoke-PSObfuscation/blob/main/layer-0-obfuscation.md#final-payload):
...
`Minusone` is developed in Rust and can be built, deployed or executed through `cargo`:

```
cargo run --features="minusone-cli" -- --path test.ps1
```

A python bindings is also available to be easily integrated, for example into `Jupyter` Notebooks. 

## What is a Rule?

A rule will produce a result when visiting a particular node, depending of children or parent. A rule will be called when entering and leaving a node.

To create a rule for `Powershell` is easier as implemented the `RuleMut` trait :
...
The `enter` function is called before visiting the node, and the `leave` function will be called when leaving the node, so after visiting the node and children.

### Example: A rule that add integer
...
The first rule we need is a rule to parse integer :
...
The rule will be process when leaving a node of type `decimal_integer_literal` in the `tree-sitter-powershell` grammar, 
then It will try to parse the token by using the `rust` `parse` method (`token.parse::<i32>()`).
...
The `Forward` rule is a particular rule that will forward node inferred type in case a node is not use in a semantic way, which is mainly due to how the `Powershell` grammar was generated.

Then, you can print an the powershell result by using the object `Linter`:
...
When using the `Engine` object, you will automatically used predefined rules designed for powershell. These can be found into `src/ps/mod.rs` :
...
* More accurate parsing of powershell hashtable
